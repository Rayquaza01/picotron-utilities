...
function sizeToReadable(sz)
	...
	if sz > ctable.G then
		return string.format("%.2f%s", sz / ctable.G, ctable.prefixes[4])
	elseif sz > ctable.M then
		return string.format("%.2f%s", sz / ctable.M, ctable.prefixes[3])
	elseif sz > ctable.K then
		return string.format("%.2f%s", sz / ctable.K, ctable.prefixes[2])
	end

	return string.format("%.2f%s", sz, ctable.prefixes[1])
end
